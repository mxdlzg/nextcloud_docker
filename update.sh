...
function create_variant() {
    ...
    url="https://download.nextcloud.com/server/prereleases/nextcloud-$fullversion.tar.bz2"
    ascUrl="https://download.nextcloud.com/server/prereleases/nextcloud-$fullversion.tar.bz2.asc"
...
curl -fsSL 'https://download.nextcloud.com/server/prereleases/' |tac|tac| \
    grep -oE 'nextcloud-[[:digit:]]+(\.[[:digit:]]+){2}(rc|beta|alpha)[[:digit:]]*' | \
    grep -oE '[[:digit:]]+(\.[[:digit:]]+){2}(rc|beta|alpha)[[:digit:]]*' | \
    sort -uV | \
    tail -1 > latest.txt
...
# Get all available prerelease versions
fullversions=( $( curl -fsSL 'https://download.nextcloud.com/server/prereleases/' |tac|tac| \
    grep -oE 'nextcloud-[[:digit:]]+(\.[[:digit:]]+){2}(rc|beta|alpha)[[:digit:]]*' | \
    grep -oE '[[:digit:]]+(\.[[:digit:]]+){2}(rc|beta|alpha)[[:digit:]]*' | \
    sort -urV ) )
versions=( $( printf '%s\n' "${fullversions[@]}" | grep -oE '^[[:digit:]]+' | sort -urV ) )
